...
#     all_obs_train = [obs1, obs2]
#     all_obs_test = [obs3, obs1[:3]] # Use some different sequences for test

#     hmm_surrogate = HMMSurrogate(n_states=2, covariance_type='diag', n_iter=50) # Using 2 states for simplicity
#     hmm_surrogate.train(all_obs_train)

#     # Test fidelity calculation
#     fidelity_results = hmm_surrogate.calculate_fidelity_metrics(all_obs_test)

#     # Decode a sequence
#     test_obs_decode = np.array([[0.8, 0.2], [0.5, 0.5], [0.3, 0.7], [0.1, 0.9]])
#     decoded_states = hmm_surrogate.decode_sequence(test_obs_decode)
#     print(f"\nTest Observation Sequence for decoding:\n{test_obs_decode}")
#     print(f"Decoded State Sequence: {decoded_states}")

#     # Analyze states
#     # Using train data for state analysis as it's more extensive for this example
#     decoded_all_train = [hmm_surrogate.decode_sequence(s) for s in all_obs_train]
#     state_analysis_results = hmm_surrogate.analyze_states(all_obs_train, decoded_all_train, target_class_idx=1) 

    # hmm_surrogate.save_model("models/dummy_hmm_fidelity.pkl")
    # loaded_hmm = HMMSurrogate().load_model("models/dummy_hmm_fidelity.pkl")
...
#     def predict_next_observation_distribution(self, obs_prefix):
#         """
#         Given obs_prefix of shape (t, D), returns the predicted next-step
#         probability vector of length D:
#           q_hat = sum_j P(S_{t+1}=j | o1..ot) * mu_j
#         """
#         if not self.is_trained:
#             raise ValueError("HMM must be trained before prediction.")

#         # ensure 2D
#         obs_prefix = np.atleast_2d(obs_prefix)
#         t, D = obs_prefix.shape
#         # --- 1) forward pass up to t ---
#         log_prob, fwdlattice = self.model._do_forward_pass(obs_prefix)
#         # normalize to get alpha_t = P(S_t=i | o1..ot)
#         # forward algorithm returns unnormalized alphas; we can normalize row t:
#         alpha_t = np.exp(fwdlattice[-1] - log_prob)   # shape (n_states,)

#         # --- 2) one-step state forecast: pi_pred = alpha_t @ A ---
#         A = self.model.transmat_                     # (K,K)
#         pi_pred = alpha_t.dot(A)                     # (K,)

#         # --- 3) mixture-of-means prediction ---
#         means = self.model.means_                    # (K, D)
#         q_hat = pi_pred.dot(means)                  # (D,)
#         # force valid prob vector
#         q_hat = np.clip(q_hat, 1e-8, 1-1e-8)
#         q_hat = q_hat / q_hat.sum()
#         return q_hat

#     def compute_fidelity(self, obs_sequences, measure='kl'):
#         """
#         Computes average divergence between the HMM's predicted next-step
#         distribution and the true next q, over all sequences and steps.
#         measure: 'kl' for KL-divergence, 'nll' for negative log-likelihood.
#         """
#         if not self.is_trained:
#             raise ValueError("HMM must be trained before computing fidelity.")
...
#                 continue
#             for t in range(T-1):
#                 q_true = seq[t+1]                        # true prob vector at t+1
#                 q_hat  = self.predict_next_observation_distribution(seq[:t+1])
#                 if measure == 'kl':
#                     # KL(q_true || q_hat)
#                     kl = np.sum(q_true * np.log(q_true / q_hat))
#                     divergences.append(kl)
#                 else:  # nll: -log q_hat[class_true], but here we have full vector
#                     # cross-entropy: - sum q_true * log q_hat
#                     ce = -np.sum(q_true * np.log(q_hat))
#                     divergences.append(ce)

#         if not divergences:
#             print("No valid steps to compute fidelity.")
#             return None

#         avg_div = float(np.mean(divergences))
#         metric = "KL-divergence" if measure=='kl' else "Cross-Entropy"
#         print(f"\n--- Fidelity ({metric}) ---")
#         print(f"Computed over {len(divergences)} steps across {len(obs_sequences)} sequences")
#         print(f"Average {metric}: {avg_div:.4f}")
#         return avg_div
